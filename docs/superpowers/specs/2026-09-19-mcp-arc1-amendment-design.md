...
| Implementation plan | `docs/superpowers/plans/2026-09-21-mcp-arc1.md` — executed 2026-09-21 by subagent-driven development (Tasks 1–21 landed on local `main` after `8428fd4`; every task spec-reviewed and code-reviewed to APPROVED; Task 22 Steps 1–3 done 2026-09-22, see the rows below; Steps 4 and 7 done 2026-09-22/23; the ChatGPT half of Step 5 waits on OpenAI) |
| Query probes | 2026-09-21 (read-only EXPLAIN ANALYZE on production, REPEATABLE READ READ ONLY, 60 s cap; cold run then warm run; rows / count ms): acceptance (lighting 27,515 leaves via IN-list, >10k searches, <500 reviews, volume desc) = 4458/69 cold, 520/87 warm, plans Bitmap Index Scan kcs_leaf_path_idx + sort; same at offset 950 = 80/63, 71/69; no-category reviews asc >10k = 691/1596, 216/318, Index Scan kcs_avg_reviews_idx (count: Bitmap kcs_est_vol_idx); growing 4w preset = 202/25340 cold, 65/40 warm, rows Index Scan Backward kcs_vol_delta_4w_idx, COUNT Seq Scan (cold 25 s → cancelled at the 3 s count cap = `unknown`; FIXED by ordering the count subquery on the delta expression so it walks the partial index); text word match + reviews bound = 2766/51, 145/50, Bitmap kcs_norm_trgm_b_idx; title gap any loose, lighting = 1615/676, 109/154, kcs_rank_idx walk + leaf bitmap; volume asc (worst first, min 424) = 8462/207, 338/39, Parallel Index Scan Backward kcs_rank_idx (SFR tie group, accepted); volume movement observed_only prior-volume bound = 135/397, 36/42, kcs_rank_idx (planner preferred the rank walk over kcs_est_vol_4w_idx — fine); averageReviews desc, whole population = 17888/30 cold, 2476/35 warm, Parallel Seq Scan + top-N (DESC NULLS LAST cannot use the ASC index → FIXED: sorting by a nullable key now excludes null keys and emits plain ASC/DESC so the index serves both directions; re-probe after 0e2f28f: 230 ms via Index Scan Backward kcs_avg_reviews_idx; low_review preset + reviews desc 879/480 via the same index / kcs_cat_cover_idx); growing 4w COUNT re-probe: 2818 ms via Index Scan Backward kcs_vol_delta_4w_idx (down from 25 s; still close to the 3 s count cap, so a cold growth-preset total may report `unknown` — honest by design); wordCount desc over the whole population = 24413 ms rows (Parallel Seq Scan: `word_count` has no index) → over the 10 s deadline → QUERY_TIMEOUT unless combined with a text/category/range filter — documented in the guide rather than adding DDL for the beta. Probe 2/3 (volumeDelta sort + selective scope): text `lamp` + growth = 2028/198 ms (trigram bitmap; count unaffected by the steering); lighting leaves + growth = 670→163 ms rows via the delta partial index, count 392 ms warm / 6955 ms cold as a BitmapAnd of kcs_leaf_path_idx and kcs_vol_delta_4w_idx (the planner's own choice with or without steering — a cold scoped growth total may report `unknown`); the count steering (`fbe5fa8`) is therefore applied only to unscoped volume-delta shapes. No category-scoped shape seq-scanned kcs. |
| Migration 0047 | APPLIED to production 2026-09-22 (about 15:45 UTC) on the owner’s confirmation, via the untracked `scripts/applyMigration0047.ts` in one transaction: `research_usage_buckets` + `research_usage_buckets_start_idx` + `mcp_connections` with `mcp_connections_status_check`, all asserted afterwards; both tables empty. Applied before any push, per the deploy-order rule. |
| Integration tests (Task 22 Step 3) | 2026-09-22, against production: `researchUsage` green (5 concurrent reservations at a limit of 3 → exactly 3 fulfilled with gapless counts 1–3, 2 `RATE_LIMITED` with a 30 s retry, bucket 5 requests / 250 rows); `volumeMonotonicity` green = 0 inversions on the live 2026-09-19 snapshot (5.2 s). Test harness landed in `b4e126e` (helpers + orphan-sweep teardown, proven by seeding one synthetic user and watching the teardown remove it) and `c763d45` (`.claude/**` excluded from vitest and eslint: the desktop app’s side-session worktrees inside the repo were being collected once per worktree). |
| Re-probe on the 2026-09-19 snapshot | 2026-09-22, after the weekly import (03:21 UTC), the kcs swap (05:34) and the Keepa run (completed 10:15 UTC, 19,588 ASINs, 0 errors; facets exist only for snapshot `33ab8a93`): all nine shapes ≤ 2.2 s rows (text word match 2.2 s, acceptance 0.9 s, volume asc 0.7 s, the rest ≤ 0.35 s) and ≤ 0.5 s count; the category-scoped volumeDelta count took 4.9 s cold (23k page reads, plan unchanged) and 315 ms warm, so the first scoped growth total after a weekly swap may report `unknown`, by design. Verification re-run on `c763d45`: typecheck clean, 101 files / 1148 unit tests, `next build` OK with the six routes in the manifest; full-repo lint shows only pre-existing React-Compiler-rule errors in UI files outside this arc’s diff (TabNav chip task_ec5e8066; the remaining five files flagged as a separate follow-up). |
| Deploy (Task 22 Step 4) | 2026-09-22 16:39 UTC: `main` pushed on the owner’s go (`4403aef..e470cbe`, 73 commits) after a clean active-jobs check; Vercel production and the Railway worker green by 16:41 UTC. Unauthenticated smoke: `POST /api/mcp` 401 with the resource-metadata challenge, `/connect-ai` 307 to sign-in for browser navigations. Authenticated smoke: `whoami` from a connected client returned the account and `datasetWeek` 2026-09-19 through the full gate, so the 0047 tables are live. Clients that cached the spike’s tool list show only `whoami` until they reconnect. |
| Fluidity checklist (§8.2) | claude.ai: PASS on all nine prompts (owner, 2026-09-23: “Everything worked fantastically in claude”); that session registered about 70 research calls between 17:04 and 19:53 UTC on 2026-09-22 in `research_usage_buckets`. ChatGPT: not yet testable — the five tools are listed, but the ChatGPT app currently stops calling any tool, for every connector, after the first tool call in a chat until a new chat is opened (owner, 2026-09-23); multi-step research cannot run there until that clears. Retest then. |
| Audience flip (Task 22 Step 7) | 2026-09-23: the owner set `RESEARCH_CURSOR_SECRET` and `MCP_AUDIENCE=all` in Vercel (`MCP_ALLOWED_CLIENT_IDS` was already pinned to the two ids) and redeployed from the dashboard; the production redeploy of `e470cbe` succeeded at 10:54 UTC. Authenticated `whoami` through the claude.ai client passed afterwards; the 401 challenge and the sign-in redirect are unchanged; the Connect AI tab is visible to every account. Arc 1 is live for all accounts. ChatGPT retest pending on OpenAI’s fix. |
