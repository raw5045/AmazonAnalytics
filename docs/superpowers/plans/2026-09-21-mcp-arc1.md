...
- [ ] **Step 7: Audience flip (OWNER-GATED)** — when the owner says the experience is fluid: set `RESEARCH_CURSOR_SECRET` (random, 32+ chars), `MCP_AUDIENCE=all`, and `MCP_ALLOWED_CLIENT_IDS=16oat62Xksi7U2Ri,WzrKBzjxqjhn2pUR` (the two ids `lib/mcp/config.ts`'s `CLIENT_LABELS` already names) in Vercel, redeploy; the Connect AI tab appears for every account. Leaving the allowlist unpinned is acceptable only while `MCP_AUDIENCE=admin`. Record the date in the amendment's Results and update the memory note (`mcp-integration.md` + `MEMORY.md` index line).
