...
> **Superseded in part by the Task 21 probe fix (`0e2f28f`):** sorting by `averageReviews` or `wordCount` adds `kcs.<col> IS NOT NULL` to the WHERE and orders with plain `ASC`/`DESC` (no `NULLS LAST`) so `kcs_avg_reviews_idx` serves both directions — a whole-population reviews-desc sort measured 17.9 s cold as a seq scan and 230 ms after the change; nulls are never ranked (documented in the guide). For `volumeDelta` sorts the count subquery gets `ORDER BY <delta expr> <dir>` before `LIMIT COUNT_CAP + 1` so it walks the 0044 partial index (25 s → 2.8 s). `wordCount` has no index: a whole-population wordCount sort still seq-scans (24 s) and the guide tells clients to combine it with a text, category or range filter. Treat HEAD as authoritative over the snippets below.
...
> **Superseded in part by the Task 15 fix round (see git log):** `search_keywords` publishes the FULL input schema (every field optional, no wire-level defaults, field-level descriptions) instead of a loose `{ cursor }` object — the "arc-2 refinement" landed in arc 1 because the model otherwise has to guess field names; the SDK rejects schema-invalid calls with prose before the tool runs. Treat HEAD as authoritative over the snippets below.
...
> **Landed after review:** `deleteStaleUsageBuckets` tolerates SQLSTATE `42P01` (relation missing → one warn + `0`) so the Railway worker's hourly cron is safe before migration 0047 is applied; any other error still propagates (Inngest retries once). The cron follows `warmExplorerLanding.ts`'s fresh-`pg.Pool` convention.
...
> **Note (Task 12, resolved 2026-09-22):** `tests/integration/helpers.ts` and `tests/integration/globalSetup.ts` were committed in `b4e126e`, with the vitest `globalSetup` wiring taken verbatim from the owner’s parked stash; the four refactored legacy integration tests from that stash remain parked.
...
> **Done 2026-09-22:** 0047 applied with `APPLY_0047=yes` (one transaction; tables, key columns and the status CHECK asserted). Both integration tests green against production — run as `RUN_INTEGRATION=1 pnpm vitest run <file>` from Git Bash, where `cross-env` is not on the PATH. Harness committed in `b4e126e`; `.claude/**` excluded from vitest and eslint in `c763d45` because the desktop app’s side-session worktrees live inside the repo and were collected once per worktree. Outcomes are in the amendment’s Results table.

- [x] **Step 4: Push (OWNER-GATED)** — on the owner's go: `node --env-file=.env.local --import tsx scripts/checkActiveJobs.ts` (no running Keepa runs, no batch in an importing state), then `git push origin main`, then watch `gh api repos/raw5045/AmazonAnalytics/commits/<sha>/status` until Vercel and the Railway worker are green. Verify live, unauthenticated: `POST https://keywordquarry.com/api/mcp` still returns the 401 challenge; `https://keywordquarry.com/connect-ai` redirects to sign-in. Then an AUTHENTICATED smoke (the unauthenticated 401 check cannot detect the post-auth 503): the owner runs `whoami` from one connected client and gets the account back; a `503 temporarily_unavailable` here most likely means 0047 is not applied (Step 3) — confirm via the Vercel log line `connection_lookup_failed` carrying `relation "mcp_connections" does not exist`, then apply it; no redeploy needed. (Any transient Neon failure in either lookup also answers 503, so check the log line before acting.)

> **Done 2026-09-22:** pushed `4403aef..e470cbe` (73 commits) at 16:39 UTC after a clean active-jobs check; Vercel production and the Railway worker both reported success by 16:41 UTC. Unauthenticated: `POST /api/mcp` = 401 with the resource-metadata challenge; `/connect-ai` = 307 to sign-in for browser navigations (Clerk answers 404 to non-navigation requests, by design). Authenticated: `whoami` from a connected client returned the account with `datasetWeek` 2026-09-19 through the full gate, so the connection lookup works against the applied 0047 tables. Clients that cached the spike’s tool list show only `whoami` until they reconnect.
...
> **Status 2026-09-23:** claude.ai passed all nine (owner: “Everything worked fantastically”). ChatGPT is untestable for now: its app stops calling any tool, for every connector, after the first tool call in a chat, so the multi-step flows cannot run there; the five tools are listed. Retest when ChatGPT fixes it.
...
- [x] **Step 7: Audience flip (OWNER-GATED)** — when the owner says the experience is fluid: set `RESEARCH_CURSOR_SECRET` (random, 32+ chars), `MCP_AUDIENCE=all`, and `MCP_ALLOWED_CLIENT_IDS=16oat62Xksi7U2Ri,WzrKBzjxqjhn2pUR` (the two ids `lib/mcp/config.ts`'s `CLIENT_LABELS` already names) in Vercel, redeploy; the Connect AI tab appears for every account. Leaving the allowlist unpinned is acceptable only while `MCP_AUDIENCE=admin`. Record the date in the amendment's Results and update the memory note (`mcp-integration.md` + `MEMORY.md` index line).

> **Done 2026-09-23:** the owner set `RESEARCH_CURSOR_SECRET` and `MCP_AUDIENCE=all` in Vercel (`MCP_ALLOWED_CLIENT_IDS` was already pinned to the two ids; order is irrelevant, the gate checks membership) and redeployed from the dashboard; the production redeploy of `e470cbe` succeeded at 10:54 UTC. Authenticated `whoami` through the claude.ai client passed afterwards, the 401 challenge and the sign-in redirect are unchanged, and the Connect AI tab now appears for every account (admins see “Audience: all” in the page’s Admin block). ChatGPT retest still pending on OpenAI’s fix.
