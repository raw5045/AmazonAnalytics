...
```ts
// lib/research/contracts.test.ts
import { describe, it, expect } from 'vitest';
...
- [ ] **Step 2: Run it and watch it fail** — `pnpm vitest run lib/research/contracts.test.ts` → cannot resolve `./contracts`.

- [ ] **Step 3: Implement the schemas**

```ts
// lib/research/contracts.ts
import { z } from 'zod';
import { ResearchError } from './errors';
...
export const movementSchema = z
  ...
  .superRefine((m, ctx) => {
    ...
    // the same floor-aware emptiness rule integerRange uses (rangeFloorIssues).
...
```ts
// lib/research/contracts.ts (continued)
...
export function parseSearchInput(raw: unknown): ParsedSearchInput {
  ...
  // A `cursor` key present with an undefined value isn't a continuation attempt; strip it
  // so it doesn't trip the strict-object unrecognized-key check below.
...
- [ ] **Step 4: Add the output types to the same file**

```ts
// lib/research/contracts.ts (continued) — output shapes (TypeScript only; the
// tools declare zod outputSchemas built from these in Task 15)
...
export interface TotalMatches { kind: 'exact' | 'at_least' | 'unknown'; value: number | null }
...
export interface PresetApplication { presetId: PresetId; appliedFields: string[]; overriddenFields: string[] }
...
export interface HistoryPoint { weekEndDate: string; rank: number; estimatedMonthlySearches: number | null; volumeIsExtrapolated: boolean; severity: Severity | null }
...
- [ ] **Step 2: Run and watch it fail** — `pnpm vitest run lib/research/limits.test.ts`.

- [ ] **Step 3: Implement, and add the env keys**

```ts
// lib/research/limits.ts
import { env } from '@/lib/env';
...
In `lib/env.ts`, directly under the `MCP_RESOURCE_URL` line:
...
And in `.env.example`, under the MCP block:

```
# ============== RESEARCH SERVICE (MCP tools) ==============
# HMAC key for search cursors; unset = derived from CLERK_SECRET_KEY. Set a random 32+ char value before the beta rollout.
RESEARCH_CURSOR_SECRET=
# Optional JSON overriding limits, e.g. {"requestsPerMinute":120}. See lib/research/limits.ts
RESEARCH_LIMITS_JSON=
```
...
- [ ] **Step 2: Run and watch it fail** — `pnpm vitest run lib/research/catalog.test.ts`.

- [ ] **Step 3: Implement (part 1: presets)**
...
export function applyPresets(request: SearchRequest): { filters: Filters; sort: Sort; comparisonWindow: Window; applications: PresetApplication[] } {
  ...
  let comparisonWindow: Window | undefined = request.comparisonWindow;
...
- [ ] **Step 3 (part 2): metric definitions and the guide**

```ts
// lib/research/catalog.ts (continued)
